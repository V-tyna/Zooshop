...
import { cleanPopUpType } from './redux/actionTypes';

function App() {
	const [isPopUp, setIsPopUp] = useState(false);
	const [keyForPopUpComponent, setKeyForPopUpComponent] = useState('');
	...
	useEffect(() => {
		...
		if (isPopUp) {
			timeout = setTimeout(() => {
				setIsPopUp(false);
				store.dispatch({ type: cleanPopUpType });
			}, 2000);
		}
		return () => {
	...
	return (
		<div>
			<Header />
			{isPopUp ? <PopUp keyPop={keyForPopUpComponent} /> : null}
			...
			<Routes>
				...
				<Route
					path='/:animals/:category/:specific/:id'
					element={<Item />}
				/>
